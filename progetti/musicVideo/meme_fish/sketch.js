...
new p5(p=>{
    const div = document.getElementById("meme_fish_div");
    let righe, res, prom = new Promise(r=>{res=r});
    ...
    p.draw = function() {
        ...
        p.stroke(0,255,0);
        p.strokeWeight(2);
        const bounce = p.createVector(0,Math.sin(p.frameCount/10)/4,0);
        for (const faccia of facce) {
            for (let i=0; i<faccia.length; i++){
                const a = vertici[faccia[i]];
                const b = vertici[faccia[(i+1)%faccia.length]];
                const at = p5.Vector.add(p5.Vector.mult(p5.Vector.add(a,bounce),p.width/1.3/(2.5+a.z)),mid_screen);
                const bt = p5.Vector.add(p5.Vector.mult(p5.Vector.add(b,bounce),p.width/1.3/(2.5+b.z)),mid_screen);
        ...
        p.text("FISH", p.width/2, p.height/5);
        p.text("FISH", p.width/2, p.height-p.height/20);
    }
